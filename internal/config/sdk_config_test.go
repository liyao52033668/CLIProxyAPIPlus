...
func TestSanitizeVertexCompatKeysTrimsDisplayName(t *testing.T) {
	cfg := &Config{VertexCompatAPIKey: []VertexCompatKey{{
		APIKey: "key",
	...
	if got := cfg.VertexCompatAPIKey[0].Models[0].DisplayName; got != "Display Name" {
		t.Fatalf("sanitized display name = %q, want Display Name", got)
	}
}
