...
func TestBuildConfigChangeDetails_SecretsAndCounts(t *testing.T) {
	oldCfg := &config.Config{
		SDKConfig: sdkconfig.SDKConfig{
			APIKeys: config.APIKeyList{{Key: "a"}},
		},
		AmpCode: config.AmpCode{
			UpstreamAPIKey: "",
		},
	...
	newCfg := &config.Config{
		SDKConfig: sdkconfig.SDKConfig{
			APIKeys: config.APIKeyList{{Key: "a"}, {Key: "b"}, {Key: "c"}},
		},
		AmpCode: config.AmpCode{
...
func TestBuildConfigChangeDetails_FlagsAndKeys(t *testing.T) {
	oldCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			...
			APIKeys:                    config.APIKeyList{{Key: "key-1"}},
	...
	newCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			...
			APIKeys:                    config.APIKeyList{{Key: " key-1 "}, {Key: "key-2"}},
...
func TestBuildConfigChangeDetails_AllBranches(t *testing.T) {
	oldCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog: false,
			ProxyURL:   "http://old-proxy",
			APIKeys:    config.APIKeyList{{Key: " keyA "}},
		},
	...
	newCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog:             true,
			ProxyURL:               "http://new-proxy",
			APIKeys:                config.APIKeyList{{Key: "keyB"}},
			DisableImageGeneration: config.DisableImageGenerationAll,
		},
