...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	s.codeArtsOAuthHandler.SetAuthFailureCallback(func(stateID string, err error) {
		management.SetOAuthSessionError(stateID, "CodeArts login failed: "+err.Error())
	})
	...
	s.joyCodeOAuthHandler.RegisterRoutes(engine, s.localOrManagementAuthMiddleware())
...
// codexAlphaSearch forwards the standalone search payload without protocol translation.
func (s *Server) codexAlphaSearch(c *gin.Context) {
	...
	body, errRead := io.ReadAll(io.LimitReader(c.Request.Body, 16<<20))
	if errRead != nil {
	...
	selected, errSelect := s.handlers.AuthManager.SelectAuthByKind(ctx, "codex", strings.TrimSpace(routing.Model), auth.AuthKindOAuth, coreexecutor.Options{
	...
	if errSelect != nil {
		...
		c.JSON(status, gin.H{"error": errSelect.Error()})
		return
	}
	...
	if _, errWrite := c.Writer.Write(upstreamBody); errWrite != nil {
		log.Errorf("codex alpha search: write response error: %v", errWrite)
	}
}
...
func (s *Server) registerManagementRoutes() {
	...
	secretExport := s.mgmt.SecretExportMiddleware()
	{
		...
		mgmt.GET("/config", secretExport, s.mgmt.GetConfig)
		mgmt.GET("/config.yaml", secretExport, s.mgmt.GetConfigYAML)
		mgmt.PUT("/config.yaml", secretExport, s.mgmt.PutConfigYAML)
		...
		mgmt.GET("/proxy-url", secretExport, s.mgmt.GetProxyURL)
		...
		mgmt.GET("/gemini-api-key", secretExport, s.mgmt.GetGeminiKeys)
		...
		mgmt.GET("/ampcode", secretExport, s.mgmt.GetAmpCode)
		mgmt.GET("/ampcode/upstream-url", secretExport, s.mgmt.GetAmpUpstreamURL)
		...
		mgmt.GET("/ampcode/upstream-api-key", secretExport, s.mgmt.GetAmpUpstreamAPIKey)
		...
		mgmt.GET("/ampcode/upstream-api-keys", secretExport, s.mgmt.GetAmpUpstreamAPIKeys)
		...
		mgmt.GET("/claude-api-key", secretExport, s.mgmt.GetClaudeKeys)
		mgmt.PUT("/claude-api-key", s.mgmt.PutClaudeKeys)
		mgmt.PATCH("/claude-api-key", s.mgmt.PatchClaudeKey)
		mgmt.DELETE("/claude-api-key", s.mgmt.DeleteClaudeKey)

		mgmt.GET("/codex-api-key", secretExport, s.mgmt.GetCodexKeys)
		mgmt.PUT("/codex-api-key", s.mgmt.PutCodexKeys)
		mgmt.PATCH("/codex-api-key", s.mgmt.PatchCodexKey)
		mgmt.DELETE("/codex-api-key", s.mgmt.DeleteCodexKey)

		mgmt.GET("/openai-compatibility", secretExport, s.mgmt.GetOpenAICompat)
		mgmt.PUT("/openai-compatibility", s.mgmt.PutOpenAICompat)
		mgmt.PATCH("/openai-compatibility", s.mgmt.PatchOpenAICompat)
		mgmt.DELETE("/openai-compatibility", s.mgmt.DeleteOpenAICompat)

		mgmt.GET("/vertex-api-key", secretExport, s.mgmt.GetVertexCompatKeys)
...
func (s *Server) SetOnCodexConfigUpdated(fn func() error) {
	...
	s.mgmt.SetOnCodexConfigUpdated(fn)
}
...
func (a *codexInspectionServiceAdapter) Run(ctx context.Context, req codexinspection.RunRequest) (codexinspection.LatestSnapshot, error) {
	return a.service.StartRun(ctx, req)
}
...
func (a *codexInspectionServiceAdapter) UpdateSettings(ctx context.Context, settings codexinspection.InspectionSettings) (codexinspection.LatestSnapshot, error) {
	snapshot, err := a.service.UpdateSettings(ctx, settings)
	if err != nil {
		return codexinspection.LatestSnapshot{}, err
	}
	if a.worker != nil {
		a.worker.Reload(snapshot.Settings, snapshot.Run.NextTriggerAtMSByProvider)
	}
	return snapshot, nil
}
