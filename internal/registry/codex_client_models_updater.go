...
// StartCodexClientModelsUpdater starts the background Codex client catalog updater.
...
func fetchCodexClientModelsFromRemote(ctx context.Context) ([]byte, string) {
	...
	for _, sourceURL := range codexClientModelsURLs {
		...
		if errRequest != nil {
			cancel()
			log.Debugf("Codex client models fetch request creation failed for %s: %v", sourceURL, errRequest)
			continue
		}
		...
		if errHTTP != nil {
			cancel()
			log.Debugf("Codex client models fetch failed from %s: %v", sourceURL, errHTTP)
			continue
		}
		...
		if errValidate := ValidateCodexClientModelsJSON(data); errValidate != nil {
			log.Warnf("Codex client models validate failed from %s: %v", sourceURL, errValidate)
			continue
		}
