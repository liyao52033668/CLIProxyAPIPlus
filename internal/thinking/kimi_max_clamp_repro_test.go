...
func TestKimiClaudeMessagesMaxClampsToHigh(t *testing.T) {
	...
	out, err := thinking.ApplyThinking(body, "kimi-k2.5", "claude", "claude", "kimi")
	if err != nil {
...
func TestKimiClaudeMessagesDisabledFallsBackToLow(t *testing.T) {
	models := registry.GetKimiModels()
	reg := registry.GetGlobalRegistry()
	clientID := "test-kimi-disabled-clamp"
	reg.RegisterClient(clientID, "kimi", models)
	t.Cleanup(func() { reg.UnregisterClient(clientID) })

	body := []byte(`{"model":"kimi-k2.7-code","messages":[{"role":"user","content":"hi"}],"thinking":{"type":"disabled"}}`)
	out, err := thinking.ApplyThinking(body, "kimi-k2.7-code", "claude", "claude", "kimi")
	if err != nil {
		t.Fatalf("ApplyThinking returned error: %v", err)
	}
	if got := gjson.GetBytes(out, "thinking.type").String(); got != "adaptive" {
