...
// This translator aggregates thinking blocks into a single reasoning item and
// text blocks into a single message item, so the round trip collapses consecutive
// thinking. The server-side search pair must still survive in order.
...
func TestClaudeWebSearchWithEmptyResultsKeepsEmptyList(t *testing.T) {
	...
	var completed gjson.Result
	for _, o := range translateClaudeResponsesStream(chunks) {
		if event, data := parseClaudeResponsesSSEEvent(t, o); event == "response.completed" {
			completed = data
		}
	}
