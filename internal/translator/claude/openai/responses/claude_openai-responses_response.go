...
func ConvertClaudeResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	case "content_block_start":
		...
		if typ == "text" {
			...
			st.ReasoningBuf.Reset()
			st.ReasoningEnc = claudeReasoningCarrier(cb)
	...
	case "content_block_delta":
		...
		if dt == "text_delta" {
			...
			if st.ReasoningActive {
				st.ReasoningEnc = d.Get("signature").String()
			}
		}
	case "content_block_stop":
		...
		if st.InTextBlock {
			...
			itemDone := []byte(`{"type":"response.output_item.done","sequence_number":0,"output_index":0,"item":{"id":"","type":"reasoning","status":"completed","encrypted_content":"","summary":[{"type":"summary_text","text":""}]}}`)
			itemDone, _ = sjson.SetBytes(itemDone, "sequence_number", nextSeq())
	...
	case "message_stop":
		...
		// reasoning item (if any). A redacted_thinking block carries no text, so
		// the carrier (signature or redacted marker) alone is enough to include it.
		if st.ReasoningBuf.Len() > 0 || st.ReasoningPartAdded || st.ReasoningEnc != "" {
...
func ConvertClaudeResponseToOpenAIResponsesNonStream(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) []byte {
	...
	for _, ch := range chunks {
		...
		case "content_block_start":
			...
			case "thinking", "redacted_thinking":
				...
				reasoningEnc = claudeReasoningCarrier(cb)
			}
		...
		case "content_block_delta":
			...
			case "signature_delta":
				if reasoningActive {
					reasoningEnc = d.Get("signature").String()
				}
			}
	...
	// Build output array
	outputsWrapper := []byte(`{"arr":[]}`)
	if reasoningBuf.Len() > 0 || reasoningEnc != "" {
		item := []byte(`{"id":"","type":"reasoning","encrypted_content":"","summary":[{"type":"summary_text","text":""}]}`)
		item, _ = sjson.SetBytes(item, "id", reasoningItemID)
		item, _ = sjson.SetBytes(item, "encrypted_content", reasoningEnc)
