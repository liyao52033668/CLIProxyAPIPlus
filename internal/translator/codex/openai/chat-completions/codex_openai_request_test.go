...
func TestCustomToolNamesStayConsistentAcrossDeclarationHistoryAndChoice(t *testing.T) {
	prefix := strings.Repeat("custom_tool_", 7)
	firstName := prefix + "first"
	secondName := prefix + "second"
	input := []byte(`{
		"messages":[
			{"role":"assistant","tool_calls":[
				{"id":"call_custom","type":"custom","custom":{"name":"` + secondName + `","input":"patch"}}
			]}
		],
		"tools":[
			{"type":"custom","name":"` + firstName + `","description":"first"},
			{"type":"custom","name":"` + secondName + `","description":"second"}
		],
		"tool_choice":{"type":"custom","name":"` + secondName + `"}
	}`)

	out := ConvertOpenAIRequestToCodex("gpt-5.6-sol", input, true)
	tools := gjson.GetBytes(out, "tools").Array()
	if len(tools) != 2 {
		t.Fatalf("expected 2 custom tools, got %d: %s", len(tools), gjson.GetBytes(out, "tools").Raw)
	}
	firstMapped := tools[0].Get("name").String()
	secondMapped := tools[1].Get("name").String()
	if firstMapped == secondMapped || len(firstMapped) > 64 || len(secondMapped) > 64 {
		t.Fatalf("expected distinct custom names <= 64 chars, got %q and %q", firstMapped, secondMapped)
	}
	if got := gjson.GetBytes(out, "input.0.name").String(); got != secondMapped {
		t.Fatalf("custom history name = %q, want declaration name %q: %s", got, secondMapped, out)
	}
	if got := gjson.GetBytes(out, "tool_choice.name").String(); got != secondMapped {
		t.Fatalf("custom tool choice name = %q, want %q: %s", got, secondMapped, out)
	}
}
