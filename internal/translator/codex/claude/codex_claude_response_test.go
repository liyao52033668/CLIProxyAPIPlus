...
// TestConvertCodexResponseToClaude_StreamThinkingClosesBlockOpenedByTextDeltaWithoutPartAdded
// covers a truncated stream that opens thinking via text.delta only and then ends the
// reasoning item without encrypted_content. The block must still stop.
func TestConvertCodexResponseToClaude_StreamThinkingClosesBlockOpenedByTextDeltaWithoutPartAdded(t *testing.T) {
	digest := digestCodexThinkingStream(t, [][]byte{
		[]byte("data: {\"type\":\"response.output_item.added\",\"item\":{\"type\":\"reasoning\"}}"),
		[]byte("data: {\"type\":\"response.reasoning_summary_text.delta\",\"delta\":\"orphan delta\"}"),
		[]byte("data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"reasoning\"}}"),
	})

	if digest.Starts != 1 || digest.Stops != 1 {
		t.Fatalf("expected one closed thinking block, got %d starts and %d stops", digest.Starts, digest.Stops)
	}
	if digest.Thinking != "orphan delta" {
		t.Fatalf("thinking text = %q, want orphan delta", digest.Thinking)
	}
	if len(digest.Signatures) != 0 {
		t.Fatalf("did not expect signature without encrypted_content, got %v", digest.Signatures)
	}
}
