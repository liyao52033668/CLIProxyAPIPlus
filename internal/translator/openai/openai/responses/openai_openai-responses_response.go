...
func ConvertOpenAIChatCompletionsResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	emitToolItem := func(key string, force bool) {
		...
		if isCustomTool {
			...
			item, _ = sjson.SetBytes(item, "item.id", fmt.Sprintf("ctc_%s", callID))
			...
			item, _ = sjson.SetBytes(item, "item.call_id", callID)
			item = applyResponsesFunctionCallNamespaceFields(item, requestForNamespace, name, "item")
			out = append(out, emitRespEvent("response.output_item.added", item))
		}
	...
	emitPendingFunctionArgs := func(key string) {
		...
		argsDelta, _ = sjson.SetBytes(argsDelta, "item_id", fmt.Sprintf("fc_%s", callID))
		argsDelta, _ = sjson.SetBytes(argsDelta, "output_index", st.FuncOutputIx[key])
		argsDelta, _ = sjson.SetBytes(argsDelta, "delta", delta)
		out = append(out, emitRespEvent("response.function_call_arguments.delta", argsDelta))
