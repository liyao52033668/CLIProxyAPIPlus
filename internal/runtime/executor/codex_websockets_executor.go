...
// cancelActiveForConn cancels the active read waiters for conn without clearing
// the channel. Used when invalidating a full terminal channel so
// sendTerminalWebsocketRead can exit via <-done instead of hanging forever
// behind a blocked consumer.
func (s *codexWebsocketSession) cancelActiveForConn(conn *websocket.Conn) {
	...
	s.activeMu.Lock()
	defer s.activeMu.Unlock()
	if s.activeConn != conn || s.activeCancel == nil {
		return
	}
	s.activeCancel()
}
...
// codexWebsocketWriteChunkSize bounds the per-Write payload size when streaming
// large websocket messages so upstream keepalive handling stays responsive.
...
func (s *codexWebsocketSession) configureConn(conn *websocket.Conn) {
	...
	conn.SetPingHandler(func(appData string) error {
		...
		// Deliberately avoid writeMu here so keepalive pongs are not starved by
		// long payload writes (also covers the xai websocket session which
		// delegates ping handling to configureConn).
...
// configureRawCodexWebsocketConn installs the keepalive ping handler on a
// connection dialed without a session object.
func configureRawCodexWebsocketConn(conn *websocket.Conn) {
	if conn == nil {
		return
	}
	conn.SetPingHandler(func(appData string) error {
		// Concurrent WriteControl is safe alongside WriteMessage; never take
		// writeMu here or keepalive pongs starve during large payload writes.
...
// newEphemeralCodexWebsocketSession builds a session object for a single
// sessionless request so it still gets ping/pong handling and structured
// disconnect logging without being registered in the shared session store.
...
func (e *CodexWebsocketsExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	for {
		...
		if eventType == "response.failed" {
			clearCodexReasoningReplayOnInvalidSignature(replayScope, payload)
		}
		...
		for {
			...
			if msgType != websocket.TextMessage {
				if msgType == websocket.BinaryMessage {
					...
					terminateErr = binaryErr
					helps.RecordAPIWebsocketError(ctx, e.cfg, "unexpected_binary", binaryErr)
					reporter.PublishFailure(ctx, binaryErr)
					if sess != nil {
						e.invalidateUpstreamConn(sess, conn, "unexpected_binary", binaryErr)
					}
					_ = send(cliproxyexecutor.StreamChunk{Err: binaryErr})
					return
				}
				continue
			}
...
func mapCodexWebsocketWriteError(sess *codexWebsocketSession, conn *websocket.Conn, err error) error {
	...
	if mapped := mapCodexWebsocketWriteErrorFromUpstream(sess, conn); mapped != nil {
		return mapped
	}
	// Write often fails with broken pipe / ErrCloseSent before the concurrent
	// reader has processed the peer's 1009 close frame. Give the close handler a
	// short window so oversized requests stay request-scoped and are not retried.
...
func mapCodexWebsocketWriteErrorFromUpstream(sess *codexWebsocketSession, conn *websocket.Conn) error {
...
// mapCodexWebsocketReadError converts an upstream 1009 (message too big) close
// into a request-scoped HTTP 413 so auth conductor neither retries other
// credentials nor applies cooldown: the request itself is too large and would
// fail identically on every credential.
...
func normalizeCodexWebsocketParallelToolCalls(body []byte, headers http.Header) []byte {
	return normalizeCodexParallelToolCalls(body, headers)
}
...
func applyCodexPromptCacheHeaders(from sdktranslator.Format, req cliproxyexecutor.Request, rawJSON []byte) ([]byte, http.Header) {
	...
	if from == "claude" {
		...
		if cached, ok, errCache := helps.ClaudeCodePromptCache(context.Background(), modelName, req.Payload, nil); errCache == nil && ok {
			cache = cached
		} else {
...
func applyCodexPromptCacheHeadersWithContext(ctx context.Context, from sdktranslator.Format, req cliproxyexecutor.Request, rawJSON []byte, headerSets ...http.Header) ([]byte, http.Header) {
	headers := http.Header{}
	if len(rawJSON) == 0 {
		return rawJSON, headers
	}
	...
	if requestHeaders == nil {
		requestHeaders = reqToHeaders(ctx)
	}
	...
	if from == "claude" {
		...
		if cached, ok, errCache := helps.ClaudeCodePromptCache(ctx, modelName, req.Payload, requestHeaders); errCache == nil && ok {
			cache = cached
		} else {
			// Preserve legacy user_id continuity for non-Claude-Code Claude clients.
			return applyCodexPromptCacheHeaders(from, req, rawJSON)
		}
	} else if from == "openai-response" {
		if promptCacheKey := gjson.GetBytes(req.Payload, "prompt_cache_key"); promptCacheKey.Exists() {
			cache.ID = promptCacheKey.String()
...
func (e *CodexWebsocketsExecutor) ensureUpstreamConn(ctx context.Context, auth *cliproxyauth.Auth, sess *codexWebsocketSession, authID string, wsURL string, headers http.Header) (*websocket.Conn, *http.Response, error) {
	if sess == nil {
		...
		if conn != nil {
			configureRawCodexWebsocketConn(conn)
		}
...
func (e *CodexWebsocketsExecutor) invalidateUpstreamConn(sess *codexWebsocketSession, conn *websocket.Conn, reason string, err error) {
	...
	// Unblock any sendTerminalWebsocketRead waiting behind a full active channel
...
func logCodexWebsocketConnectedWithReused(sess *codexWebsocketSession, sessionID string, authID string, wsURL string, reused bool) {
	...
	sessionKind := sessionObjectKind(sess)
	log.Infof("codex websockets: upstream connected session=%s auth=%s url=%s session_object=%s reused=%t", sessionStr, strings.TrimSpace(authID), strings.TrimSpace(wsURL), sessionKind, reused)
}
...
func logCodexWebsocketDisconnectedWithLastEvent(sess *codexWebsocketSession, sessionID string, authID string, wsURL string, reason string, lastEvent string, err error) {
	...
	if err != nil {
		...
		log.Infof("codex websockets: upstream disconnected session=%s auth=%s url=%s session_object=%s reason=%s is_terminal=%t err=%v", sessionStr, strings.TrimSpace(authID), strings.TrimSpace(wsURL), sessionKind, strings.TrimSpace(reason), terminalStatus, err)
		return
	}
	if lastEvent != "" {
	...
	log.Infof("codex websockets: upstream disconnected session=%s auth=%s url=%s session_object=%s reason=%s is_terminal=%t", sessionStr, strings.TrimSpace(authID), strings.TrimSpace(wsURL), sessionKind, strings.TrimSpace(reason), terminalStatus)
}
