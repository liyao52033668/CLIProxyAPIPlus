...
func TestResolveDevinSessionAndCascadeIDs(t *testing.T) {
	...
	// 3. Fallback to fresh UUID when nothing supplied (local stub: no ctx session extraction)
	sidCtx, cidCtx := resolveDevinSessionAndCascadeIDs(context.Background(), "", "", cliproxyexecutor.Options{})
