...
func configureXAIWebsocketConn(sess *codexWebsocketSession, conn *websocket.Conn) {
	// Reuse the shared Codex session configure path so close-handler 1009 capture
	// and write-error mapping stay in lockstep between providers. XAI previously
	// used a zero pong deadline; WriteControl(deadline zero) means "no deadline",
	// which is equivalent for this keepalive reply.
