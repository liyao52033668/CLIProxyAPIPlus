...
func claudeThinkingReplayScopeFromRequest(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) claudeThinkingReplayScope {
	sessionKey := ""
	if executionID := helps.MetadataString(opts.Metadata, cliproxyexecutor.ExecutionSessionMetadataKey); executionID != "" {
		sessionKey = "execution:" + executionID
	} else if executionID := helps.MetadataString(req.Metadata, cliproxyexecutor.ExecutionSessionMetadataKey); executionID != "" {
		sessionKey = "execution:" + executionID
	} else {
