...
func parseDevinDataURL(raw string) (mimeType string, data string, ok bool) {
	...
	extractFromPart := func(p gjson.Result) {
		...
		case "image", "input_image", "image_url":
			...
			if base64Data == "" {
				...
				if m, d, ok := parseDevinDataURL(url); ok {
					base64Data = d
					if mimeType == "" {
						mimeType = m
					}
...
func resolveDevinSessionAndCascadeIDs(ctx context.Context, sessionID, cascadeID string, opts cliproxyexecutor.Options) (string, string) {
	// TODO: port upstream session ID infrastructure (util.SessionIDFromContext, cliproxyauth.CanonicalSessionID)
	// For now, fall back to ExtractSessionID from headers/payload/metadata.
	if sessionID == "" {
		if canon := cliproxyauth.ExtractSessionID(opts.Headers, opts.OriginalRequest, opts.Metadata); canon != "" {
			sessionID = canon
		}
	}
