...
func (e *AntigravityExecutor) refreshToken(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	result, errRefresh, _ := antigravityRefreshGroup.Do(refreshToken, func() (interface{}, error) {
		// Use a detached context with a bounded timeout so the OAuth refresh is
		// not cancelled when the caller disconnects, but also cannot hold the
		// connection pool indefinitely under high client churn.
