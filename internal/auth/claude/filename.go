...
func FindMatchingLegacyCredential(ctx context.Context, store coreauth.Store, target *coreauth.Auth) (*coreauth.Auth, error) {
	...
	// Use provider-scoped listing when the store supports it to avoid loading
	// all auth records (e.g. Postgres full-table scan) just to find Claude ones.
	var (
		records []*coreauth.Auth
		errList error
	)
	if pl, ok := store.(coreauth.ProviderLister); ok {
		records, errList = pl.ListByProvider(ctx, "claude")
	} else {
