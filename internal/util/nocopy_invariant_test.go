...
var reviewedInPlaceByteWrites = map[string]reviewedInPlaceByteWrite{
	"internal/auth/cursor/proto/connect.go":       {1, "copies header bytes into a freshly allocated frame; no payload buffer is rewritten"},
	"internal/usage/keeper/api/usage_overview.go": {18, "zeroes fields on internally owned usage series maps; no payload buffer is rewritten"},
	"internal/usage/keeper/repository/usage.go":   {9, "zeroes fields on internally owned usage series maps; no payload buffer is rewritten"},
	"internal/usage/usage.go":                     {17, "shifts an internally owned events slice or zeroes usage series map fields; no payload buffer is rewritten"},
}
