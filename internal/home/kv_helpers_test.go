...
func TestCurrentKVClientUnavailableErrors(t *testing.T) {
	...
	disabled := New(config.HomeConfig{Enabled: false}, 0, 0)
	SetCurrent(disabled)
	...
	notReady := New(config.HomeConfig{Enabled: true, Host: "127.0.0.1", Port: 1}, 0, 0)
	SetCurrent(notReady)
	if _, homeMode, errClient := CurrentKVClient(); !homeMode || errClient == nil {
...
func TestKVRequiredHelpersPropagateClientErrors(t *testing.T) {
	client := New(config.HomeConfig{Enabled: true, Host: "127.0.0.1", Port: 1}, 0, 0)
