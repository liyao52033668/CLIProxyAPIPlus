...
func MergeRefreshedAuth(base, current, updated *Auth) *Auth {
	...
	if finalDisabled {
		...
		if disabledFlag, _ := merged.Metadata["disabled"].(bool); !disabledFlag {
		...
		// Only flip an existing disabled flag; do not inject the key into
		// metadata that never carried it.
		if disabledFlag, ok := merged.Metadata["disabled"].(bool); ok && disabledFlag {
